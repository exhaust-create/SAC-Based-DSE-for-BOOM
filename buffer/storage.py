...
class RolloutStorage(object):
    def __init__(self, mem_size, obs_shape, action_space):
        """
        Description
        -------
        ...
        self.rewards = torch.zeros(mem_size, 1)
        self.value_preds = torch.zeros(mem_size + 1, 1)
        self.returns = torch.zeros(mem_size + 1, 1)
    ...
    def insert(self, obs, option, action, action_log_prob, value_pred, reward, mask):
        ...
        if self.step >= self.mem_size:
            ...
            self.value_preds[self.step].copy_(value_pred.view(1))
    ...
    def normalize_rewards(self):
        ...
        norm_reward = (reward - reward.mean()) / (reward.std() + 1e-8)
        self.rewards[:min(self.step, self.mem_size)] = norm_reward
    ...
    def compute_returns(self, last_value, gamma, use_gae=False, tau=0.95):
        """
        Description
        -------
