...
class SAC:
    ''' 处理离散动作的SAC算法 '''
    def __init__(self, config, env, device='cpu', embedding=True):
        self.config = config
        self.env = env
        self.device = device
        ...
        if embedding:
            ...
            self.critic_1 = Q_Critic_NN(input_dim=env.observation_space.n, output_dim = self.env.action_space.n, device=device)
            self.critic_2 = Q_Critic_NN(input_dim=env.observation_space.n, output_dim = self.env.action_space.n, device=device)
            self.target_critic_1 = Q_Critic_NN(input_dim=env.observation_space.n, output_dim = self.env.action_space.n, device=device)
            self.target_critic_2 = Q_Critic_NN(input_dim=env.observation_space.n, output_dim = self.env.action_space.n, device=device)
        ...
        self.actor_scheduler = optim.lr_scheduler.StepLR(self.actor_optimizer, step_size=25, gamma=self.lr_gamma)

        self.critic_1_optimizer = optim.Adam(params = self.critic_1.parameters(), lr=2*self.lr)
        self.critic_1_scheduler = optim.lr_scheduler.StepLR(self.critic_1_optimizer, step_size=25, gamma=self.lr_gamma)

        self.critic_2_optimizer = optim.Adam(params = self.critic_2.parameters(), lr=2*self.lr)
        self.critic_2_scheduler = optim.lr_scheduler.StepLR(self.critic_2_optimizer, step_size=25, gamma=self.lr_gamma)
        ...
        if embedding:
            ...
            self.state_embed_scheduler = optim.lr_scheduler.ExponentialLR(self.state_embed_optimizer, gamma=self.lr_gamma)

        self.target_entropy = -torch.tensor(0.6*np.log(1/self.env.action_space.n))  # 目标熵的大小
    ...
    def run_an_episode(self):
        ...
        while not done:
            ...
            with torch.no_grad():
                ...
                self.target_value.append(state_value.item() + self.gamma*reward)    # For test
                self.n_step_buffer.compute_returns(state_value, self.gamma)
                ...
                # NOTE: Seems MIN, MAX or MEAN has no much impact on the results.
    ...
    def update_model(self):
        ...
        for _ in range(train_iter):
            # Sampling
            indices, states, options, actions, rewards, returns, next_states, terminals, IS_weights = self.all_transitions.sample_buffer(priority=True)
