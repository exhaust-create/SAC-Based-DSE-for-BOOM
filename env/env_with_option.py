...
class BOOMENV(gym.Env):
   ...
   def get_state_reward(self, state):
      ...
      if not self._explored_designs:
         reward = proj_with_punishment - self._init_reward
      else:
         current_sum_reward = np.array([row[2] for i, row in enumerate(self._explored_designs)]).sum()
         reward = proj_with_punishment - current_sum_reward    # Culculate step reward.
   ...
   def normalize_ppa(self):
      normalized_ppa = (self._ppa - self._first_microarch_ppa)/self._first_microarch_ppa
      return normalized_ppa

   def renormalize_ppa(self, state):
      return state*self._first_microarch_ppa + self._first_microarch_ppa
   ...
   def get_best_point(self, final_microarchs, normalized_ppas, episode_rewards):
      ...
      proj_with_punishment = episode_rewards.transpose().squeeze(0) - punishment
